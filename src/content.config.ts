...
// 后台（Sveltia CMS）把未填的可选字段写成空字符串而不是省略，
// 这里统一把 '' 归一化为 undefined，避免合法的保存被校验拦截。
...
const workSchema = z.object({
	...
	updatedDate: emptyToUndef(z.coerce.date().optional()),
	tags: z.array(z.string()).default([]),
	// 为 true 时详情页才加载 KaTeX 样式，纯文字页不背负公式 CSS
	math: z.boolean().default(false),
	// 完整源码仓库链接（代码类作品常用）
	githubUrl: emptyToUndef(z.string().url().optional()),
	// 附件 PDF（如 LaTeX 笔记原版）在 public/files/ 下的路径
	pdfUrl: emptyToUndef(z.string().optional()),
